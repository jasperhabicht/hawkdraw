...
\msg_new:nnn { hawkdraw } { text-mode-unknown } {
    Text ~ mode ~ `#1` ~ unknown.
}
...
\int_new:N \l__hawkdraw_node_text_mode_int
\bool_new:N \l__hawkdraw_node_text_mode_clip_bool
\tl_new:N \l__hawkdraw_node_text_mode_clip_contents_tl
...
\prop_const_from_keyval:Nn \c__hawkdraw_text_mode_prop {
    fill                 = 0 ,
    stroke               = 1 ,
    fill ~ stroke        = 2 ,
    none                 = 3 ,
...
\keys_define:nn { hawkdraw / path / node } {
    ...
    text ~ mode             .code:n         = {
        \prop_if_in:NnTF \c__hawkdraw_text_mode_prop {#1} {
            \int_set:Nn \l__hawkdraw_node_text_mode_int {
                \prop_item:Nn \c__hawkdraw_text_mode_prop {#1}
            }
            \int_compare:nNnT {
                \l__hawkdraw_node_text_mode_int
            } > { 3 } {
                \bool_set_true:N \l__hawkdraw_node_text_mode_clip_bool
            }
        } {
            \msg_warning:nnn { hawkdraw } { text-mode-unknown } {#1}
        }
    } ,
}
...
\cs_new_protected:Npn \hawkdraw_node:nn #1#2 {
    \draw_scope_begin:
        ...
        \bool_if:NTF \l__hawkdraw_node_text_mode_clip_bool {
            \tl_put_right:Nn \l__hawkdraw_node_text_mode_clip_contents_tl {
                \hawkdraw_support_pdfliteral:n {
                    \int_use:N \l__hawkdraw_node_text_mode_int \c_space_tl Tr ~
                }
        ...
        \bool_if:NT \l__hawkdraw_node_text_mode_clip_bool {
...
\cs_new_protected:Npn \__hawkdraw_node_set_frame:n #1 {
    \hcoffin_set:Nn \l_hawkdraw_node_frame_coffin {
        \draw_suspend_begin:
            ...
            \draw_begin:
                \bool_if:NT \l__hawkdraw_node_text_mode_clip_bool {
                    \tl_use:N \l__hawkdraw_node_text_mode_clip_contents_tl
                }
