...
\tl_new:N \l_hawkdraw_text_render_mode_tl

\keys_define:nn { hawkdraw / text } {
    render ~ mode           .tl_set:N       = \l_hawkdraw_text_render_mode_tl ,
    render ~ mode           .initial:n      = { fill } ,
}

\prop_const_from_keyval:Nn \c__hawkdraw_text_render_prop {
    fill                 = 0 ,
    stroke               = 1 ,
    fill ~ stroke        = 2 ,
    none                 = 3 ,
...
\cs_new_protected:Npn \hawkdraw_text_render_init_check: {
    \str_if_eq:VnF \l_hawkdraw_text_render_mode_tl { fill } {
        \int_const:Nn \c__hawkdraw_text_render_stack_int {
...
\cs_new_protected:Npn \hawkdraw_text_render_stack_push:n #1 {
    \int_if_exist:NF \c__hawkdraw_text_render_stack_int {
        \hawkdraw_text_render_init_check:
    }
    \int_if_exist:NT \c__hawkdraw_text_render_stack_int {
        \hawkdraw_support_pdfcolorstack:e {
            \int_use:N \c__hawkdraw_text_render_stack_int ~ push ~ {
                \prop_item:Nn \c__hawkdraw_text_render_prop {#1} ~ Tr ~
            }
        }
    }
}
\cs_generate_variant:Nn \hawkdraw_text_render_stack_push:n { V }

\cs_new_protected:Npn \hawkdraw_text_render_stack_pop: {
    \int_if_exist:NT \c__hawkdraw_text_render_stack_int {
        \hawkdraw_support_pdfcolorstack:e {
            \int_use:N \c__hawkdraw_text_render_stack_int ~ pop ~ { }
        }
    }
}

% Inserts something at the end if `stroke' is set
% Using clip inside node does not result in clipping (same with `pdfrender'). Why? Keep like that?
% Should this functionality be incorporated into `node`? This way text could be placed freely.

\NewDocumentCommand { \hawkdrawtext } { O{} +m } {
    \group_begin:
        \clist_clear:N \l__hawkdraw_keys_unprocessed_clist
        \keys_set_known:nnN { hawkdraw / path } {#1}
            \l__hawkdraw_keys_unprocessed_clist
        \keys_set:nV { hawkdraw / text }
            \l__hawkdraw_keys_unprocessed_clist
        \hawkdraw_text_render_stack_push:V \l_hawkdraw_text_render_mode_tl
        #2
        \hawkdraw_text_render_stack_pop:
    \group_end:
}
