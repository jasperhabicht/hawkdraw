...
\keys_define:nn { hawkdraw / scope } {
    text ~ render ~ mode    .code:n         = {
        \prop_if_in:NnTF \c__hawkdraw_text_render_mode_prop {#1} {
            \hawkdraw_text_render_mode_stack_push:e {
                \prop_item:Nn \c__hawkdraw_text_render_mode_prop {#1}
            }
            \hook_gput_next_code:nn { hawkdraw / scope / end } {
                \hawkdraw_text_render_mode_stack_pop:
            }
        } {
...
\prop_const_from_keyval:Nn \c__hawkdraw_text_render_mode_prop {
    fill                 = 0 ,
    stroke               = 1 ,
    fill ~ stroke        = 2 ,
    none                 = 3 ,
...
\cs_new_protected:Npn \hawkdraw_text_render_mode_stack_push:n #1 {
    \int_if_exist:NF \c__hawkdraw_text_render_mode_stack_int {
        \int_if_zero:nF {#1} {
            \int_const:Nn \c__hawkdraw_text_render_mode_stack_int {
    ...
    \int_if_exist:NT \c__hawkdraw_text_render_mode_stack_int {
        \hawkdraw_support_pdfcolorstack:e {
            \int_use:N \c__hawkdraw_text_render_mode_stack_int ~ push ~ {
                #1 ~ Tr ~
            }
        }
    }
}
\cs_generate_variant:Nn \hawkdraw_text_render_mode_stack_push:n { e }

\cs_new_protected:Npn \hawkdraw_text_render_mode_stack_pop: {
    \int_if_exist:NT \c__hawkdraw_text_render_mode_stack_int {
        \hawkdraw_support_pdfcolorstack:e {
            \int_use:N \c__hawkdraw_text_render_mode_stack_int ~ pop \scan_stop:
        }
    }
}
...
\cs_new_protected:Npn \hawkdraw_node_contents:n #1 {
    ...
    \normalfont
    \clist_if_empty:NF \l_hawkdraw_node_text_keys_clist {
