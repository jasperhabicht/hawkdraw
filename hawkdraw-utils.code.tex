...
\cs_new_protected:Npn \hawkdraw_support_pdf_write_stream:nnn #1#2#3 {
    % not working.
    % #3 is stream in hex, but probably #1 is wrong
...
\cs_generate_variant:Nn \hawkdraw_support_pdf_write_stream:nnn { nne }
...
\cs_new:Npn \hawkdraw_hex_to_stream:n #1 {
    \sys_if_engine_xetex:TF {
        ...
        \__hawkdraw_hex_to_stream:w #1 \q_stop
    }
}
\cs_generate_variant:Nn \hawkdraw_hex_to_stream:n { e }

\cs_new:Npn \__hawkdraw_hex_to_stream:w #1 \q_stop {
     \__hawkdraw_hex_to_stream:NNw #1 \q_stop
}

\cs_new:Npn \__hawkdraw_hex_to_stream:NNw #1#2#3 \q_stop {
    \sys_if_engine_luatex:T {
        \lua_now:e {
            token.put_next (
                token.new (
    ...
    \tl_if_empty:nF {#3} {
        \__hawkdraw_hex_to_stream:w #3 \q_stop
    }
}
