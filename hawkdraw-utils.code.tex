...
% Calculation of local bbox

% ===

\fp_new:N \l_hawkdraw_softpath_bbox_point_last_fp

\dim_new:N \g_hawkdraw_softpath_bbox_xmin_dim
\dim_new:N \g_hawkdraw_softpath_bbox_xmax_dim
\dim_new:N \g_hawkdraw_softpath_bbox_ymin_dim
\dim_new:N \g_hawkdraw_softpath_bbox_ymax_dim

\cs_new_protected:Npn \hawkdraw_softpath_bbox_reset: {
    \dim_gset:Nn \g_hawkdraw_softpath_bbox_xmin_dim { \c_max_dim }
    \dim_gset:Nn \g_hawkdraw_softpath_bbox_xmax_dim { -\c_max_dim }
    \dim_gset:Nn \g_hawkdraw_softpath_bbox_ymin_dim { \c_max_dim }
    \dim_gset:Nn \g_hawkdraw_softpath_bbox_ymax_dim { -\c_max_dim }
}

\cs_new_protected:Npn \hawkdraw_softpath_bbox_update:nn #1#2 {
    \dim_gset:Nn \g_hawkdraw_softpath_bbox_xmin_dim {
        \dim_min:nn { \g_hawkdraw_softpath_bbox_xmin_dim } {
            \fp_to_dim:n {#1}
        }
    }
    \dim_gset:Nn \g_hawkdraw_softpath_bbox_xmax_dim {
        \dim_max:nn { \g_hawkdraw_softpath_bbox_xmax_dim } {
            \fp_to_dim:n {#1}
        }
    }
    \dim_gset:Nn \g_hawkdraw_softpath_bbox_ymin_dim {
        \dim_min:nn { \g_hawkdraw_softpath_bbox_ymin_dim } {
            \fp_to_dim:n {#2}
        }
    }
    \dim_gset:Nn \g_hawkdraw_softpath_bbox_ymax_dim {
        \dim_max:nn { \g_hawkdraw_softpath_bbox_ymax_dim } {
            \fp_to_dim:n {#2}
        }
    }
}

\cs_new_protected:Npn \hawkdraw_softpath_bbox_update_fn: {
    \hawkdraw_softpath_bbox_reset:
    \fp_set:Nn \l_hawkdraw_softpath_bbox_point_last_fp { 0pt , 0pt }
    \clist_map_inline:nn {
        ...
        \cs_set_eq:cc {
            ...
            hawkdraw_softpath_bbox_ ##1
        }
    }
    \tl_use:N \g_hawkdraw_softpath_original_tl
}

\cs_new_protected:Npn \hawkdraw_softpath_bbox_moveto_op:nn #1#2 {
    \hawkdraw_softpath_bbox_update:nn {#1} {#2}
    \fp_set:Nn \l_hawkdraw_softpath_bbox_point_last_fp { #1 , #2 }
}

\cs_new_protected:Npn \hawkdraw_softpath_bbox_lineto_op:nn #1#2 {
    \hawkdraw_softpath_bbox_update:nn {#1} {#2}
    \fp_set:Nn \l_hawkdraw_softpath_bbox_point_last_fp { #1 , #2 }
}

\cs_new_protected:Npn \hawkdraw_softpath_bboxn_curveto_op:nnNnnNnn #1#2#3#4#5#6#7#8 {
    \hawkdraw_softpath_bbox_update:nn {#1} {#2}
    \hawkdraw_softpath_bbox_update:nn {#4} {#5}
    \tl_if_empty:nTF {#7} {
        \fp_set:Nn \l_hawkdraw_softpath_bbox_point_last_fp { #4 , #5 }
    } {
        \hawkdraw_softpath_bbox_update:nn {#7} {#8}
        \fp_set:Nn \l_hawkdraw_softpath_bbox_point_last_fp { #7 , #8 }
    }
}

\cs_new:Npn \hawkdraw_softpath_bbox_angle_normal:n #1 {
    \int_mod:nn { \int_mod:nn {#1} { 360 } + 360 } { 360 }
}

% #1 : test angle
% #2 : start angle
% #3 : end angle
\prg_new_conditional:Npnn \hawkdraw_softpath_bbox_angle_between:nnn #1#2#3 { TF } {
    % if start <= end
    \int_compare:nNnTF {
        \hawkdraw_softpath_bbox_angle_normal:n {#2}
    } > {
        \hawkdraw_softpath_bbox_angle_normal:n {#3}
    } {
...
\cs_new_protected:Npn \hawkdraw_softpath_bbox_arc_op:nnNnn #1#2#3#4#5 {
    % bbox must be at least rectangle defined by starting / end point
    \hawkdraw_softpath_bbox_update:nn {
        \hawkdraw_tuple_use_i:n {
            \__hawkdraw_point_part_arc:nnnnnn
                { 1 } { \l_hawkdraw_softpath_bbox_point_last_fp }
                {#1} {ä2} {#4} {#5}
        }
    } {
    ...
    % at each quarter, bbox might be equal to center +/- radius
    \hawkdraw_softpath_bbox_angle_between:nnnT { 0 } {#4} {#5} {
        \hawkdraw_softpath_bbox_update:nn {#1} { 0pt }
    }
    \hawkdraw_softpath_bbox_angle_between:nnnT { 90 } {#4} {#5} {
        \hawkdraw_softpath_bbox_update:nn { 0pt } {#2}
    }
    \hawkdraw_softpath_bbox_angle_between:nnnT { 180 } {#4} {#5} {
        \hawkdraw_softpath_bbox_update:nn { - ( #1 ) } { 0pt }
    }
    \hawkdraw_softpath_bbox_angle_between:nnnT { 270 } {#4} {#5} {
        \hawkdraw_softpath_bbox_update:nn { 0pt } { - ( #2 ) }
    }
}

\cs_new_protected:Npn \hawkdraw_softpath_bbox_arc_axes_op:nnNnnNnn #1#2#3#4#5#6#7#8 {
    % bbox must be at least rectangle defined by starting / end point
    \hawkdraw_softpath_bbox_update:nn {
        \hawkdraw_tuple_use_i:n {
            \__hawkdraw_point_part_arc_axes:nnnnnn
    ...
    % at each quarter, bbox might be equal to center +/- radius
    % we test for atand(ax,bx) and atand(ay,by) instead
    \hawkdraw_softpath_bbox_angle_between:nnnT { atand( #4 , #7 ) } {#7} {#8} {
        \hawkdraw_softpath_bbox_update:nn {
            \hawkdraw_tuple_use_i:n {
                \__hawkdraw_point_part_arc_axes:nnnnnn
                    { atand( #4 , #7 ) } { \l_hawkdraw_softpath_bbox_point_last_fp }
                    {#1} {ä2} {#4} {#5}
            }
        } {
            \hawkdraw_tuple_use_ii:n {
    ...
    \hawkdraw_softpath_bbox_angle_between:nnnT { atand( #4 , #7 ) + 180 } {#7} {#8} {
        \hawkdraw_softpath_bbox_update:nn {
            \hawkdraw_tuple_use_i:n {
                \__hawkdraw_point_part_arc_axes:nnnnnn
                    { atand( #4 , #7 ) + 180 } { \l_hawkdraw_softpath_bbox_point_last_fp }
                    {#1} {ä2} {#4} {#5}
            }
        } {
            \hawkdraw_tuple_use_ii:n {
    ...
    \hawkdraw_softpath_bbox_angle_between:nnnT { atand( #5 , #8 ) } {#7} {#8} {
        \hawkdraw_softpath_bbox_update:nn {
            \hawkdraw_tuple_use_i:n {
                \__hawkdraw_point_part_arc_axes:nnnnnn
                    { atand( #5 , #8 ) } { \l_hawkdraw_softpath_bbox_point_last_fp }
                    {#1} {ä2} {#4} {#5}
            }
        } {
            \hawkdraw_tuple_use_ii:n {
    ...
    \hawkdraw_softpath_bbox_angle_between:nnnT { atand( #5 , #8 ) + 180 } {#7} {#8} {
        \hawkdraw_softpath_bbox_update:nn {
            \hawkdraw_tuple_use_i:n {
                \__hawkdraw_point_part_arc_axes:nnnnnn
                    { atand( #5 , #8 ) + 180 } { \l_hawkdraw_softpath_bbox_point_last_fp }
                    {#1} {ä2} {#4} {#5}
            }
        } {
            \hawkdraw_tuple_use_ii:n {
...
\cs_new_protected:Npn \hawkdraw_softpath_bbox_circle_op:nnNnn #1#2#3#4#5 {
    \hawkdraw_softpath_bbox_update:nn { ( #1 ) - ( #4 ) } { ( #2 ) - ( #4 ) }
    \hawkdraw_softpath_bbox_update:nn { ( #1 ) + ( #4 ) } { ( #2 ) + ( #4 ) }
}

\cs_new_protected:Npn \hawkdraw_softpath_bbox_ellipse_op:nnNnnNnn #1#2#3#4#5#6#7#8 {
    \hawkdraw_softpath_bbox_update:nn {
        ( #1 ) - sqrt( ( #4 ) ^ 2 + ( #7 ) ^ 2 )
    } {
        ( #2 ) - sqrt( ( #5 ) ^ 2 + ( #8 ) ^ 2 )
...
\cs_new_protected:Npn \hawkdraw_softpath_bbox_rectangle_op:nnNnn #1#2#3#4#5 {
    \hawkdraw_softpath_bbox_update:nn {#1} {#2}
    \hawkdraw_softpath_bbox_update:nn {#4} {#5}
    \fp_set:Nn \l_hawkdraw_softpath_bbox_point_last_fp { #4 , #5 }
}

\cs_new_protected:Npn \hawkdraw_softpath_bbox_grid_op:nnNnnNnn #1#2#3#4#5#6#7#8 {
    \hawkdraw_softpath_bbox_update:nn {#4} {#5}
    \hawkdraw_softpath_bbox_update:nn {#7} {#8}
    \fp_set:Nn \l_hawkdraw_softpath_bbox_point_last_fp { #7 , #8 }
}

\cs_new_protected:Npn \hawkdraw_softpath_bbox_close_op:nn #1#2 { }

% ===

\tl_new:N \l__hawkdraw_path_bbox_tl

\cs_new_protected:Npn \hawkdraw_path_set_bbox:NNNN #1#2#3#4 {
    \group_begin:
        \hawkdraw_softpath_update:Nn \l__hawkdraw_path_bbox_tl { bbox }
        \tl_use:N \l__hawkdraw_path_bbox_tl
    \group_end:
    \dim_set_eq:NN #1 \g_hawkdraw_softpath_bbox_xmin_dim
    \dim_set_eq:NN #2 \g_hawkdraw_softpath_bbox_xmax_dim
    \dim_set_eq:NN #3 \g_hawkdraw_softpath_bbox_ymin_dim
    \dim_set_eq:NN #4 \g_hawkdraw_softpath_bbox_ymax_dim
}

\cs_new_protected:Npn \hawkdraw_path_gset_bbox:NNNN #1#2#3#4 {
    \group_begin:
        \hawkdraw_softpath_update:Nn \l__hawkdraw_path_bbox_tl { bbox }
        \tl_use:N \l__hawkdraw_path_bbox_tl
    \group_end:
    \dim_gset_eq:NN #1 \g_hawkdraw_softpath_bbox_xmin_dim
    \dim_gset_eq:NN #2 \g_hawkdraw_softpath_bbox_xmax_dim
    \dim_gset_eq:NN #3 \g_hawkdraw_softpath_bbox_ymin_dim
    \dim_gset_eq:NN #4 \g_hawkdraw_softpath_bbox_ymax_dim
}
