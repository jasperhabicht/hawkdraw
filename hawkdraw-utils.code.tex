...
\cs_new_protected:Npn \hawkdraw_support_pdfxform:nnN #1#2#3 {
    ...
    \sys_if_engine_xetex:T {
        \hook_gput_next_code:nn { shipout / background } {
            \mode_leave_vertical:
            \tex_special:D {
                pdf: bxobj ~ @hawkdraw \int_use:N \g__hawkdraw_support_pdfxform_int \c_space_tl 
                width  ~ \box_wd:N #3 \c_space_tl
                height ~ \box_ht:N #3 \c_space_tl
                depth  ~ \box_dp:N #3
            }
...
\cs_new_protected:Npn \hawkdraw_support_pdfxform_use:N #1 {
    ...
    \sys_if_engine_xetex:T {
        \tex_special:D {
            pdf: uxobj ~ @hawkdraw \int_use:N #1 \scan_stop:
        }
    }
}
\cs_generate_variant:Nn \hawkdraw_support_pdfxform_use:N { c }

\cs_new_protected:Npn \hawkdraw_support_pdfxform_ref:N #1 {
    \bool_lazy_or:nnT {
        \sys_if_engine_luatex_p:
    } {
        \sys_if_engine_pdftex_p:
    ...
    \sys_if_engine_xetex:T {
        \tex_special:D {
            pdf: obj ~ @hawkdraw \int_use:N #1 \scan_stop:
        }
    }
}
