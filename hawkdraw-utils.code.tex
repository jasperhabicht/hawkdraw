...
\cs_new_protected:Npn \hawkdraw_support_pdfxform:nnN #1#2#3 {
    ...
    \sys_if_engine_xetex:T {
        % Copy to a new temporary box in order to set its size to zero
        ...
        % Unlike other implementations, we do not delay to after shipout
        % as this is not necessary here and can easily break references
        \tex_special:D {
            pdf: bxobj ~
