...
\def\hawkdrawfiledate{17 June 2026}
...
\changes{v0.3.1}{2026/06/17}{Allowing for multiple softpath manipulations.}
...
\end{macrodef}
In order to create offset paths for Bézier curves, arcs and ellipses, the relevant offset path is approximated using a number of cubic Bézier curves. The default number of curve segments to approximate a Bézier curve or an arc up to \num{90} degrees is \num{3}. Arcs that have a delta angle larger than \num{90} degrees as well as ellipses are split up into parts of at most \num{90} degrees. The number of curve segments can be adjusted using the \macro{setting/length precision} option.
