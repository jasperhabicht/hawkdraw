...
\def\hawkdrawfileversion{0.0.7}
\def\hawkdrawfiledate{25 May 2026}
...
\changes{v0.0.7}{2026/05/25}{Adding patterns; adding \titlemacro{at path part} option.}
...
\begin{macrodef}[key:atpathpart]
!path/point/!|at path part|={<float>}
\end{macrodef}
The position of a point can also be adjusted using the \macro{at path part} option which accespts a floating-point number that defines the position on the last path segment with \num{0} being the previous and \num{1} being the current point.

\begin{codeexamplecolumns}
\begin{hawkdraw}
  \path[stroke] (0cm,0cm)
    -- (2cm,1cm)
    point[name={A}, at path part={0}]
    point[name={B}, at path part={0.5}]
    point[name={C}, at path part={0.9}] ;
...
\begin{macrodef}
!path/tip/!|at path part|={<float>}
\end{macrodef}
The position of the arrow head can be adjusted with the \macro{at path part} option which accespts a floating-point number (see \hyperlink{key:atpathpart}{above}).

\begin{codeexamplecolumns}
\begin{hawkdraw}
  \stroke (0cm,0cm) -- (2cm,1cm)
    tip[at path part={0.25}] {straight} ;
\end{hawkdraw}
\end{codeexamplecolumns}
...
\begin{hawkdraw}
  \stroke (0cm,0cm) -- (2cm,1cm)
    tip[at path part={0.25}, flipped] {straight} ;
\end{hawkdraw}
\end{codeexamplecolumns}
