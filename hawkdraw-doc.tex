...
\def\hawkdrawfileversion{0.0.9}
\def\hawkdrawfiledate{1 June 2026}
...
\changes{v0.0.9}{2026/06/01}{Adding tagging support.}
...
\end{macrodef}
To enable more complex tagging scenarios, the \macro{tagging-setup} option can be used to configure the tagging behavior. It accepts as options the same options as described above. In addition, it accepts the option \macro{tag} to define the element tag. For example, the setting \macro{tagging-setup={alt={Water (H20)}, tag={Formula}}} would tag the \macro{hawkdraw} environment as a block element with the tag \macro{Formula} and set the replacement text to ``Water (H20)''.
