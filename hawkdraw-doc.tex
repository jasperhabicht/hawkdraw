...
\def\hawkdrawfileversion{0.1.1}
\def\hawkdrawfiledate{3 June 2026}
...
\subsection{Setting the baseline, the layers and the bounding box}
...
The following path operations require one argument in a pair of square brackets (which may be empty). As for options, the relevant operator only accepts the relevant subset to the \macro{path/} set.
...
|point|[<options>]
\end{macrodef}
Using the \macro{point} operation, a point at the current position is defined and named with the given name. As for options, the \macro{point} operator only accepts the \macro{path/point/} subset. The \macro{point} operator requires one argument in a pair of square brackets (which may be empty).
...
\subsection{Stroke and fill color, opacity, clipping}
