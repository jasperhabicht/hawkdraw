...
\def\hawkdrawfileversion{0.0.8}
\def\hawkdrawfiledate{27 May 2026}
...
\makedoctitle
  ...
  {Jasper Habicht}[E-mail: \href{mailto:mail@jasperhabicht.de}{mail@jasperhabicht.de}. I am grateful to Joseph Wright, Max Chernoff, Mikael Clea F. Rees, Persson Sundqvist and Jonathan P. Spratte who helped me improving the code and the math behind it. Victor, the drawing hawk: \textcopyright{} 2026 Hannah Klöber.]
...
\changes{v0.0.8}{2026/05/27}{Improving mapping functionality; adding slope option for nodes; adding mechanism to place arrow heads on paths; adding \titlemacro{at part} option for circles, ellipses and rectangles.}
...
\end{macrodef}
The position of a point can also be adjusted using the \macro{at part} option which accespts a floating-point number that defines the position on the last path segment with \num{0} being the previous and \num{1} being the current point.
...
\end{macrodef}
Using the \macro{flipped} option, the node can be rotated by additional 180 degrees if it is sloped. The default value is \macro{false}.
...
|map|[<options>]{<code>}
\end{macrodef}
The \macro{map} operation is used to map over a comma-separated list of items or a range of numbers and execute code for each item that is given by the relevant argument. The code can reference the current item with \macro{#1} and the current index with \macro{#2}. The following options can be set for the command:
...
\begin{hawkdraw}
  \map[start={30}, step={120}, end={270}] {
    [fill] (#1:0.75cm) circle[radius={5pt}]
  } ;
\end{hawkdraw}
\end{codeexamplecolumns}
...
\end{macrodef}
The position of the arrow head can be adjusted with the \macro{at part} option which accespts a floating-point number (see \hyperlink{key:atpart}{above}).
...
|\lastpoint|
\end{macrodef}
The commands \macro{\firstpoint} and \macro{\lastpoint} can be used to access the first and last point of the current path, respectively.

\begin{macrodef}
|\firstslope|
|\lastslope|
\end{macrodef}
The commands \macro{\firstslope} and \macro{\lastslope} can be used to access the slope at the first and last point of the current path, respectively. The slope is represented as the angle with respect to the positive x-axis.
