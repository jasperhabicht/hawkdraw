...
\def\hawkdrawfileversion{0.4.0}
\def\hawkdrawfiledate{27 June 2026}
...
\changes{v0.4.0}{2026/06/27}{Adding shadings.}
...
\warning The \macro{hawkdraw} package supports PDFLaTeX and LuaLaTeX in full. Most basic commands should work with other compilation engines as well. Some features such as shadings or complex opacity settings, however, are not supported with LaTeX (via DVI) or XeLaTeX.
...
\begin{longtable}{ @{} p{2.5cm} p{\dimexpr\textwidth-2\tabcolsep-2.5cm} @{} }
  ...
  \macro{clist} & A comma-separated list is a list of strings that are separated by commas. Such a list can cosnist of no or only one item as well. If the relevant list can only have a maximum number of items, this is stated in the description of the relevant option. Note that a comma-separated list is typically not surrounded by parenthesis. \\
...
|\hawkdrawset|{<options>}
\end{macrodef}
The command \macro{\hawkdrawset} is used to set options. It can be used inside or outside of \macro{hawkdraw} environments. It uses the \macro{path/} set of options. If options of other sets should be called, they need to be called from the root level.

For example options of the \macro{picture/} set can be called using \macro{\hawkdrawset{picture/<option>}}.
...
The factor by which the slope vector is scaled can be adjusted using the option \macro{plot/smooth/tension} which is set to \num{0.15} by default
...
The \macro{hawkdraw} packages makes use of a softpath mechanism that allows for manipulations of points of a paths. The mechanism for rounding path corners via the \macro{corner arc} option (see \hyperlink{key:cornerarc}{above}) relies on a lower-level softpath implementation of \macro{l3draw}. Currently, a mechanism for shortening paths and for offsetting paths are provided by the \macro{hawkdraw} which is described below.

Note that the order of manipulations is executed in the order the relevant options are given to the path. Thus, if a path should be shortenend and also offset using a list (creating essentially two partial paths), the options for shortening should be given before the option for offsetting the path.
...
Note that the offset paths for curves, ellipses and arcs are approximated mostly by reconstructing the relevant path by three to twelve Bézier curves which may result in suboptimal output especially for relatively sharp curves. Furthermore, the position of the point where two path segmnts meet is approximated for curves and arcs.
...
\end{macrodef}
The option \macro{pattern tile size} accepts a tuple of two dimensions representing the size of the pattern tile. The default size of a pattern tile is \qty{10}{\point}\,\texttimes\,\qty{10}{\point}.
...
Note that in order to be able to use opacity and shadings, \macro{\DocumentMetadata{}} needs to be set.
...
\iffalse
\subsection{Soft masks} %%% Work in progress

\begin{macrodef}
!scope/!|soft mask set|={<string>}
\end{macrodef}
The option \macro{soft mask set} defines the content of the current scope as greyscale soft mask with the name given as string value. 

\begin{macrodef}
!scope/!|soft mask use|={<string>}
\end{macrodef}
The option \macro{soft mask use} sets the soft mask of the current scope. The option takes as string the name of the previously defined soft mask. 
Note that the options \macro{soft mask set} and \macro{soft mask use} are only available for scopes.

\begin{codeexamplecolumns}
\begin{hawkdraw}
  \begin{scope}[
    soft mask set={custom}
  ]
    \fill[color fill={gray}] (0cm,0cm)
      circle[radius={0.5cm}] ;
    \fill[color fill={black}] (-1cm,-0.25cm) 
      rectangle[corner={1cm,0.25cm}] ;
  \end{scope}
  \begin{scope}[
    soft mask use={custom}
  ]
    \fill[color fill={blue}] (0.75cm,0cm)
...
\end{macrodef}
The option \macro{blend mode} sets the blend mode of the current scope. The key takes as string one of the following values. Note that the option \macro{blend mode} is only available for scopes.
...
\subsection{Linear and radial shadings}

\begin{macrodef}
!path/!|fill shading|={<string>}
\end{macrodef}
The option \macro{fill shading} sets a fill shading for the current path. The key takes as value a string representing the name of the relevant shading.
...
\NewDocumentCommand{\shadingexample}{ m }{%
  \begin{hawkdraw}[baseline={0.5ex}]
    \path[fill shading={#1}, shading colors={red,blue,green}]
      (0cm,0cm) rectangle[corner={(2cm,1cm)}] ;
  \end{hawkdraw}%
}
