...
\def\hawkdrawfileversion{0.9.1}
\def\hawkdrawfiledate{23 September 2026}
...
\changes{v0.9.0}{2026/09/07}{Adding option to apply shadings to strokes; fixing bug in pattern module; fixing bug in setting miter limit; fixing bug in positioning of node contents.}
\changes{v0.9.1}{2026/09/24}{Adding expansion to parsing mechanism.}
...
Paths are defined by a sequence of points and path construction operations. The following sections describe the syntax for defining paths and the available operations. Note that commands used in a path definition need to be expandable in order for the parser to work properly. This is especially true for parts of path definitions that are stored in user defined commands for later use.

The following example shows how to store a (partial) path definition in an expandable command using \macro{\NewExpandableDocumentCommand}.
\begin{codeexamplecolumns}
\NewExpandableDocumentCommand{\drawcircle}{}{
  circle[radius={0.5cm}]
}
