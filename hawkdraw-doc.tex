...
It is also possible to reference a point where a line starting from the center of the node and having a specific angle intersects with the node's border. For example, if a node is named \macro{foo}, the point where a line with an angle of \num{30} degrees intersects with the node's border can be referenced by \macro{(foo[30])}.
