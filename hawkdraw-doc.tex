...
\changes{v0.4.1}{2026/07/05}{Enhacing pattern support; adding conic shadings.}
...
\warning The \macro{hawkdraw} package supports PDFLaTeX and LuaLaTeX in full. Most of the basic commands should work with other compilation engines as well. Some features such as shadings or complex opacity settings, however, are not supported with LaTeX (via DVI) or XeLaTeX.
...
\end{macrodef}
Arrows can be styled by passing the relevant path options to the options \macro{arrow start style set} and \macro{arrow end style set}. The option \macro{arrow style set} sets the same style for start and end arrow tips.
...
For example, \macro{tagging-setup={alt={Water (H20)}, tag={Formula}}} would tag the \macro{hawkdraw} environment as a block element with the tag \macro{Formula} and set the replacement text to ``Water (H20)''.
