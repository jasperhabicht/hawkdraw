...
\def\hawkdrawfileversion{0.7.1}
\def\hawkdrawfiledate{20 August 2026}
...
\end{macrodef}
The option \macro{pattern type} accepts a comma-separated list of strings representing the types of patterns that should be used for the current path. The available pattern types are \macro{fill} and \macro{stroke}. The default pattern type is \macro{fill}.
...
!global/!|bidi|={<boolean>}
\end{macrodef}
The \macro{hawkdraw} package supports bidirectionality when used with LuaTeX as bidirectional typesetting support is built into this engine. To enable it, load the package with the \macro{bidi} option in the document preamble.

In general, a right-to-left context also affects horizontal dimensions which in turn affect how paths are rendered. This means that a \macro{hawkdraw} environment should placed in a left-to-right context while the contents of nodes should retain the right-to-left context. With LuaTeX and the above option active, these changes are done automatically.

For other compilation engines such as PDFLaTeX or XeLaTeX, the following code can be inserted into the document preamble to implement a similar technique inserting commands to set and unset the direction into the relevant hooks.
