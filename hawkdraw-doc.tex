...
|\hawkdrawset|{<options>}
\end{macrodef}
The command \macro{\hawkdrawset} is used to set options. It can be used inside or outside of \macro{hawkdraw} environments. It uses the \macro{path/} set of keys. If keys of other sets should be called, they need to be called from the root level. For example keys of the \macro{picture/} set can be called using \macro{\hawkdrawset{picture/<key>}}.
...
\subsection{Options to the main user environment}
...
The following path operations require at least one option to be set. As for options, the relevant operator only accepts the relevant subset to the \macro{path/} set.
