...
It is also possible to reference a point where a line starting from the center of the node and having a specific angle intersects with the node's border. For example, if a node is named \macro{foo}, the point where a line with an angle od \num{30} degrees intersects with the node's border can be referenced by \macro{(foo[30])}.
...
\end{macrodef}
Using the options \macro{color text fill} and \macro{color text stroke}, the fill and stroke colors of the node contents are set specifically. Using the options \macro{opacity text fill} and \macro{opacity text stroke}, the fill and stroke opacity of the node contents are set specifically.
...
!path/!|pattern|={<string>}
\end{macrodef}
The option \macro{pattern} fills the current path with a pattern. The key takes as value a string representing the name of the relevant pattern.
