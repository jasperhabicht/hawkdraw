...
The \macro{hawkdraw} packages makes use of a softpath mechanism that allows for manipulations of points of a paths. The mechanism for rounding path corners via the \macro{corner arc} option relies on a lower-level softpath implementation of \macro{l3draw}. Currently, only a mechanism for shortening paths is provided by the \macro{hawkdraw} which is described below.
