...
\cs_new_protected:Npn \hawkdraw_arrow_put_single:nnn #1#2#3 {
    \tl_put_right:Nn \l_hawkdraw_path_postactions_tl {
        \hawkdraw_arrow_put:nnnn {#3}
            {
                \use:c { __hawkdraw_point_part_ \l__hawkdraw_path_type_str :V }
                    \l_hawkdraw_tip_at_path_part_fp
            }
            {
                \use:c { __hawkdraw_point_part_slope_ \l__hawkdraw_path_type_str :V }
                    \l_hawkdraw_tip_at_path_part_fp 
...
\cs_new_protected:cpn { __hawkdraw_process_path_ t :w } tip #1#2 \s__hawkdraw_stop {
    \cs_if_exist_use:cTF {
        __hawkdraw_process_path_tip_ \tl_head:n {#1} :w
    } {
        #1#2 \s__hawkdraw_stop
    } {
        \hawkdraw_arrow_put_single:nVn { }
            \l_hawkdraw_tip_at_path_part_fp {#1}
        \tl_trim_spaces_apply:nN {#2} \__hawkdraw_continue_path:n
    }
}

\fp_new:N \l_hawkdraw_tip_at_path_part_fp
\bool_new:N \l_hawkdraw_tip_flipped_bool

\keys_define:nn { hawkdraw / path / tip } {
    at ~ path ~ part        .fp_set:N       = \l_hawkdraw_tip_at_path_part_fp ,
    at ~ path ~ part        .initial:n      = { 1 } ,
    flipped                 .bool_set:N     = \l_hawkdraw_tip_flipped_bool ,
    flipped                 .default:n      = { true } ,
}

\cs_new_protected:cpn { __hawkdraw_process_path_tip_ [ :w } [ #1 ] #2#3 \s__hawkdraw_stop {
    ...
    \hawkdraw_arrow_put_single:eVn { \clist_use:N \l__hawkdraw_keys_unprocessed_clist }
        \l_hawkdraw_tip_at_path_part_fp {#2}
