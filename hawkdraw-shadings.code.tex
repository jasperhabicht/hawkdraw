...
\cs_new_protected:Npn \hawkdraw_shading_set:nnn #1#2#3 {
    ...
    \hawkdraw_support_pdfxform:nnN { } {
        % not need to set up a pattern dictionary
...
\seq_new:N \l__hawkdraw_shading_conic_colors_seq
\seq_new:N \l__hawkdraw_shading_conic_colors_hex_seq

\cs_new_protected:Npn \hawkdraw_shading_conic_stream:N #1 {
    \tl_set:Ne #1 {
        \seq_map_indexed_function:NN
            \l__hawkdraw_shading_conic_colors_hex_seq
...
\cs_new:Npn \__hawkdraw_shading_conic_stream_aux:nn #1#2 {
    ...
    \int_compare:nNnF {#1} = { 1 } {
        % flag (4 bits)
        \hawkdraw_hex_to_binary:n { 00 }
        % patch is pie segment going from 0 to 360 / ( n * 4 )
        % points (12 points, 2 * 16 bits each)
        \prg_replicate:nn { 2 } {
            \__hawkdraw_shading_conic_stream_point_polar:nn { 0 } {
                360 / ( \seq_count:N \l__hawkdraw_shading_conic_colors_hex_seq - 1 ) * ( #1 - 2 )
            }
        }
        \prg_replicate:nn { 2 } {
        ...
        \__hawkdraw_shading_conic_stream_point_polar:nn { 1 } {
            360 / ( \seq_count:N \l__hawkdraw_shading_conic_colors_hex_seq - 1 ) * ( #1 - 1.75 )
        }
        \__hawkdraw_shading_conic_stream_point_polar:nn { 1 } {
            360 / ( \seq_count:N \l__hawkdraw_shading_conic_colors_hex_seq - 1 ) * ( #1 - 1.25 )
        }
        \prg_replicate:nn { 2 } {
            \__hawkdraw_shading_conic_stream_point_polar:nn { 1 } {
                360 / ( \seq_count:N \l__hawkdraw_shading_conic_colors_hex_seq - 1 ) * ( #1 - 1 )
            }
        }
        \prg_replicate:nn { 3 } {
            \__hawkdraw_shading_conic_stream_point_polar:nn { 0 } {
        ...
        % colors (4 colors, 3 * 8 bits each)
        \hawkdraw_hex_to_binary:e {
            \seq_item:Nn \l__hawkdraw_shading_conic_colors_hex_seq { #1 - 1 }
        }
        \hawkdraw_hex_to_binary:e {
            \seq_item:Nn \l__hawkdraw_shading_conic_colors_hex_seq { #1 - 1 }
        }
        \hawkdraw_hex_to_binary:n {#2}
        \hawkdraw_hex_to_binary:e {#2}
    }
}
...
\cs_new_protected:Npn \hawkdraw_shading_color_set_hex_values:n #1 {
    \seq_clear:N \l__hawkdraw_shading_conic_colors_hex_seq
    \seq_set_from_clist:Nn \l__hawkdraw_shading_conic_colors_seq {#1}
    \seq_map_indexed_inline:Nn \l__hawkdraw_shading_conic_colors_seq {
        \int_compare:nNnTF {##1} = { 1 } {
            % first entry is the start color
            \color_export:enN {
                \seq_item:Nn \l__hawkdraw_shading_conic_colors_seq { 1 }
            } { HTML } \l__hawkdraw_shading_color_value_tmp_tl
            \seq_put_right:NV \l__hawkdraw_shading_conic_colors_hex_seq
                \l__hawkdraw_shading_color_value_tmp_tl
        } {
            \color_export:enN {
                \seq_item:Nn \l__hawkdraw_shading_conic_colors_seq { ##1 - 1 }
                ! 75 ! ##2
            } { HTML } \l__hawkdraw_shading_color_value_tmp_tl
            \seq_put_right:NV \l__hawkdraw_shading_conic_colors_hex_seq
                \l__hawkdraw_shading_color_value_tmp_tl
...
\cs_new_protected:Npn \hawkdraw_shading_conic:n #1 {
    \int_if_exist:cF {
        ...
        \hawkdraw_shading_color_set_hex_values:n {#1}
