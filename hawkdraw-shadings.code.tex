...
% Two corners of the patch are at the center of the circle, the other to at the outer border forming a curve.
...
% We need to output a binary stream. Although it is theoretically possinle to output a hex stream and set the
% relevant /Filter. But when PDFLaTeX compresses the streams, it simply appends a /Filter entry to the
% attributes of the PDF object without taking into account of other filters which results in faulty syntax.
...
\cs_new_protected:Npn \hawkdraw_shading_conic_stream:N #1 {
    ...
    \exp_args:Ne
    \tl_analysis_log:n { \seq_use:Nn \l__hawkdraw_shading_color_values_seq { , } }
}
    ...
    \int_compare:nNnF {#1} = { 1 } {
        ...
        \prg_replicate:nn { 2 } {
            \__hawkdraw_shading_conic_stream_point_polar:nn { 0 } {
                360 / ( \seq_count:N \l__hawkdraw_shading_color_values_seq - 1 ) * ( #1 - 2 )
            }
        }
        \prg_replicate:nn { 2 } {
        ...
        \__hawkdraw_shading_conic_stream_point_polar:nn { 1 } {
            360 / ( \seq_count:N \l__hawkdraw_shading_color_values_seq - 1 ) * ( #1 - 1.75 )
        }
        \__hawkdraw_shading_conic_stream_point_polar:nn { 1 } {
            360 / ( \seq_count:N \l__hawkdraw_shading_color_values_seq - 1 ) * ( #1 - 1.25 )
        }
        \prg_replicate:nn { 2 } {
            \__hawkdraw_shading_conic_stream_point_polar:nn { 1 } {
                360 / ( \seq_count:N \l__hawkdraw_shading_color_values_seq - 1 ) * ( #1 - 1 )
            }
        }
        \prg_replicate:nn { 3 } {
            \__hawkdraw_shading_conic_stream_point_polar:nn { 0 } {
    ...
    \seq_map_indexed_inline:Nn \l__hawkdraw_shading_color_values_tmp_seq {
        \int_compare:nNnTF {##1} = { 1 } {
            ...
            \__hawkdraw_shading_color_set_value:Nnnn #1 {
                \fp_to_decimal:n {
                    0.75 * \l__hawkdraw_shading_color_values_tmp_seq { ##1 - 1 } +
                    0.25 * ##2
                }
            } {
            ...
            \__hawkdraw_shading_color_set_value:Nnnn #1 {
                \fp_to_decimal:n {
                    0.5 * \l__hawkdraw_shading_color_values_tmp_seq { ##1 - 1 } +
                    0.5 * ##2
                }
            } {
                \seq_item:Nn \l__hawkdraw_shading_color_values_tmp_seq { ##1 - 1 }
            ...
            \__hawkdraw_shading_color_set_value:Nnnn #1 {
                \fp_to_decimal:n {
                    0.25 * \l__hawkdraw_shading_color_values_tmp_seq { ##1 - 1 } +
                    0.75 * ##2
                }
            } {
                \seq_item:Nn \l__hawkdraw_shading_color_values_tmp_seq { ##1 - 1 }
